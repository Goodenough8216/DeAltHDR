...
def apply_hdr_preprocessing(frames_np, start_frame_idx, exposure_ratio=8.0, gamma=1/2.2):
    """
    Apply HDR input preprocessing as described in the paper (Section 3.1, Eq. 1):
      1. Inverse gamma correction to linearize sRGB input: L_lin = L^2.2
      2. Normalize long-exposure frames to match short-exposure brightness.
      3. Concatenate linearized frame with its gamma-transformed version:
         L_c = [L', (L')^gamma]  ->  6-channel output per frame.

    Exposure sequence: {EV-2, EV+1, EV-2, ...}, so frames at even absolute
    indices are short-exposure and frames at odd indices are long-exposure.
    With a 3-stop difference, exposure_ratio = 2^3 = 8.

    Args:
        frames_np: numpy array (N, H, W, C), uint8 values in [0, 255] OR
                   float32 already in [0,1] (for DAVIS noise-added frames).
        start_frame_idx: absolute index of the first frame in the video.
        exposure_ratio: long/short exposure ratio (default 8 for 3 stops).
        gamma: gamma correction exponent for the second channel set (default 1/2.2).

    Returns:
        Preprocessed numpy array (N, H, W, 2*C), float32 values in [0, 1].
    """
    if frames_np.dtype == np.uint8:
        frames = frames_np.astype('float32') / 255.0
    else:
        frames = frames_np.astype('float32')

    # Step 1: inverse gamma correction (sRGB -> linear): L_lin = L^2.2
    frames_lin = np.power(np.clip(frames, 0.0, 1.0), 2.2)

    # Step 2: normalize long-exposure frames (multiply by 1/ratio to reduce brightness)
    n = frames_lin.shape[0]
    frames_norm = frames_lin.copy()
    for i in range(n):
        abs_idx = start_frame_idx + i
        is_long_exposure = (abs_idx % 2 == 1)  # odd index = long exposure (EV+1)
        if is_long_exposure:
            frames_norm[i] = frames_norm[i] / exposure_ratio

    frames_norm = np.clip(frames_norm, 0.0, 1.0)

    # Step 3: gamma-transform the normalized linear frames (Eq. 1: L_c = [L', (L')^gamma])
    frames_gamma = np.power(frames_norm, gamma)

    # Concatenate along channel axis: (N, H, W, C) + (N, H, W, C) -> (N, H, W, 2C)
    frames_out = np.concatenate([frames_norm, frames_gamma], axis=-1).astype('float32')
    return frames_out
