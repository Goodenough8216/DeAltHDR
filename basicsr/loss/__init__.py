...
class VGGPerceptualLoss(torch.nn.Module):
    def __init__(self, device, resize=True):
        ...
        # 冻结 VGG16 的参数（必须遍历 parameters()，不是 modules）
        for bl in blocks:
    ...
    def forward(self, input, target, swd=False, feature_layers=[0, 1, 2, 3], style_layers=[]):
        ...
        # target 的 VGG 特征不需要梯度（no_grad）
        # model output (input) 的特征需要保留梯度图以便反传
        with torch.no_grad():
            y_feats = []
            yt = y
            for block in self.blocks:
        ...
        for i in feature_layers:
            loss = loss + torch.nn.functional.l1_loss(x_feats[i], y_feats[i].detach())

        for i in style_layers:
            act_x = x_feats[i].reshape(x_feats[i].shape[0], x_feats[i].shape[1], -1)
            act_y = y_feats[i].detach().reshape(y_feats[i].shape[0], y_feats[i].shape[1], -1)
            gram_x = act_x @ act_x.permute(0, 2, 1)
            gram_y = act_y @ act_y.permute(0, 2, 1)
            loss = loss + torch.nn.functional.l1_loss(gram_x, gram_y)
